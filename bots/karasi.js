...
async function stopAllTokens() {
  for (const [token, client] of activeClients) {
    try {
      ...
      console.log(`Karasi token ${token.substring(0, 10)}... stopped`);
...
async function startTokens() {
  try {
    const tokens = await tokenService.getKarasiTokens();
     for (const token of tokens) {
       try {
    const client = new Client(); 
       await client.login(token).then(async () => {
    const eventsFolder = path.join(__dirname, '..', 'tokensHandler', 'Karasi', 'events');
    const eventFiles = fs.readdirSync(eventsFolder).filter(file => file.endsWith('.js')); 
      ...
      console.log(`Karasi token ${token.substring(0, 10)}... started successfully`);
       }).catch(error => {
      console.error(`Failed to login with token ${token.substring(0, 10)}...:`, error.message);
        ...
        console.log('Karasi tokens loading process completed!');
    } catch (error) {
